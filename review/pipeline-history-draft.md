# Draft pipeline release history (release-first backbone)

Review date: 2026-07-18. This is an evidence draft for later integration into
`references/pipeline-history.md`; it is not yet a claim that every feature was
delivered for every MOUS processed in an interval.
...
1. The guides document `manifest.xml`, not the exact filename grammar and
   schema of every delivered `*.pipeline_manifest.xml`; inspect packages
   before declaring a field stable across releases.
2. No reviewed release guide establishes the introduction or archive-delivery
   prevalence of `pipeline-*.selfcal.json`, `pipeline-*.timetracker.json`,
   `pipeline_stats_*.json`, or `*.pldriver_report.xml`. Existing skill claims
   about these are empirical and need package/query provenance.
...
4. The release docs do not establish the README-to-QA2-report transition date
   or all historical directory placements. QA2 Data Products documents and
